<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <title>Casas de Cuidado Diario</title>
    
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="author" content="Agencia Mixen www.mixen.mx">
    <meta name="description" content="Casas de Cuidado Diario es un modelo alternativo para el cuidado, atención y protección de los niños pequeños hijos de madres trabajadoras que no cuentan con un servicio adecuado para dejar a sus hijos durante su jornada laboral.">
    <meta name="keywords" content="Cuidado de niños, niñas, casas de cuidado, casas para niños, hogar para niños, escuelas para niños de bajos recursos, madres cuidadoras de niños">
   
   <!-- favicon -->
   <link rel="shortcut icon" href="img/favicon.ico" type="image/x-icon">
    
    <!-- CSS -->
    <link rel="stylesheet" href="css/bootstrap.min.css">
    <link rel="stylesheet" href="css/bootstrap-theme.min.css">
    <link rel="stylesheet" href="css/style.css">
</head>
<body>
 <script>
  (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
  (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
  m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
  })(window,document,'script','//www.google-analytics.com/analytics.js','ga');

  ga('create', 'UA-62936918-1', 'auto');
  ga('send', 'pageview');

</script>
  <header>
        <nav class="navbar navbar-default" role="navigation">
            <div class="container">
                <div class="navbar-header">
                    <a href="index.php" class="navbar-brand">
                        <img src="img/logo.jpg" alt="Casas de cuidado Diario logo" class="img-responsive">
                    </a>
                    <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target=".navbar-collapse">
                     <span class="sr-only">Toggle navigation</span>
                     <span class="icon-bar"></span>
                     <span class="icon-bar"></span>
                     <span class="icon-bar"></span>
                    </button>
                </div>
                <div class="navbar-contact">
                    <p class="text-right">
                        Tel. (614) 413.4300<br /><br />
                        Tel. (614) 413.0319<br /><br />
                        comunicacion@casasdecuidadodiario.org
                    </p>
                </div>
            </div>
            <div class="container-fluid nav-border-top">
                <div class="row">
                  <div class="col-lg-2 nav-hidden"></div>
                  <div class="col-lg-8 col-md-12">
                       <div class="collapse navbar-collapse">
                          <ul class="nav nav-justified">
                              <li class="border-menu"><a href="index.php">Home</a></li>
                              <li class="border-yellow"><a href="nosotros.php">About Us</a></li>
                              <li class="border-green"><a href="vinculacion.php">Services</a></li>
                              <li class="border-light-yellow"><a href="madres-cuidadoras.php">Care Mothers</a></li>
                              <li class="border-orange"><a href="sorteo.php">Raffle</a></li>
                              <li class="border-teal"><a href="donaciones.php">Donations</a></li>
                              <li class="border-blue"><a href="ubicaciones.php">Locations</a></li>
                              <li class="border-purple"><a href="voluntariado.php">Volunteers</a></li>
                              <li class="border-pink"><a href="contacto.php">Contact</a></li>
                          </ul>
                       </div>
                   </div>
                  <div class="col-lg-2 nav-hidden"></div>
                </div>
            </div>
        </nav> 
    </header>